...
BarWidget {
  ...
  readonly property bool revealInactiveIndicators: alwaysShowIndicators
  // Collapse to nothing (no pill) when no Omarchy indicator is active.
  visible: root.activeIndicatorIds.length > 0 || root.alwaysShowIndicators
  ...
  component IndicatorLoader: Item {
    ...
    function injectProps() {
      ...
      if ("useActiveColor" in target) target.useActiveColor = true
      if ("activeColor" in target) target.activeColor = "#a55555"
    }
