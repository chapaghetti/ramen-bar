...
BarWidget {
  ...
  // Integer-snapped reveal extent. Driving the chevron, the clip window and
  // the drawer block from this keeps every moving edge on a whole pixel, so
  // glyphs and icon textures are never sampled at fractional offsets during
  // the reveal animation (which made the tray icon and drawer apps shimmer).
  readonly property int revealExtentPx: Math.round(root.revealExtent)
  ...
  Component {
    ...
    Item {
      ...
      readonly property int drawerBlockWidth: root.allItems.length > 0 ? expandIcon.implicitWidth + root.revealExtentPx : 0
      ...
      Item {
        ...
        Item {
          ...
          width: root.revealExtentPx
          height: root.barSize
          clip: true

Row {
          id: trayIcons
          // Keep the drawer icons pinned at their resting positions: the
          // clip window simply grows over them, so the apps stay visually
          // stationary while the drawer expands and contracts. (Translating
          // this row each frame made the icons jitter on the capsule's
          // leading edge.)
          x: 0
          anchors.verticalCenter: parent.verticalCenter
  ...
  Component {
    ...
    Item {
      ...
      readonly property int drawerBlockHeight: root.allItems.length > 0 ? expandIcon.implicitHeight + root.revealExtentPx : 0
